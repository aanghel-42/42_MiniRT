...
/*   Updated: 2023/02/19 15:37:26 by pcatapan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	create_file(char *name, int i, int j)
{
	...
	path = 0;
	bmpname = (char *)ft_ec_malloc(ft_strlen(name) + 8);
	while (name[i])
		if (name[i++] == '/')
			path++;
	i = 0;
	...
	if (!((fd = open(bmpname, O_WRONLY | O_CREAT | O_TRUNC,
													S_IRUSR | S_IWUSR)) > 0))
		ft_fatal("in do_the_bmp_thing() while creating file");
	free(bmpname);
	return (fd);
}
...
void		write_file(int fd, t_scene data, t_minilibx mlx)
{
	...
	pixel_array = (char *)ft_ec_malloc(mlx.cam->size_line * data.yres);
