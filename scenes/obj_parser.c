...
void		*ft_ec_malloc(size)
{
	void	*ptr;

	ptr = malloc(size);
	if (ptr == NULL)
		ft_fatal("in ec_malloc() on memory allocation");
	return (ptr);
}
...
int main(int ac, char **av)
{
	...
	if (!((fd = open(av[1], O_RDONLY)) > 0))
		ft_fatal("in main() while opening file");
	name = (char *)ft_ec_malloc(strlen(av[1]));
	for (i = 0; av[1][i] && av[1][i] != '.'; i++)
		name[i] = av[1][i];
	strcat(name, ".rt");
	//printf("name is %s\n", name);
	if (!((fd2 = open(name, O_WRONLY | O_CREAT | O_APPEND, S_IRUSR | S_IWUSR)) > 0))
		ft_fatal("in main() while creating file");
